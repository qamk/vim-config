...
if not cmp_status_ok then
    vim.notify("nvim-cmp not found! Is it installed properly?")
    return
end

local snip_status_ok, luasnip = pcall(require, "luasnip")
if not snip_status_ok then
    vim.notify("luasnip not found! Is it installed? If you are using another snippet engine change in completion.lua as well")
    return
end
...
cmp.setup {
  ...
  sources = {
      { name = "nvim_lsp", keyword_length = 4, max_item_count = 30 },
      { name = "nvim_lua" },
      -- { name = "luasnip" },
      { name = "buffer" },
      { name = "path" },
